...
======================================
Custom training loop for SafeLoRA and SaLoRA baselines on GSM8K or Alpaca.

Usage:
  python experiments/run_baselines.py --method safelora --task gsm8k --seed 42
  python experiments/run_baselines.py --method salora --task gsm8k --seed 42
"""
...
from src.baselines import apply_salora, safelora_eval_context, log_salora_stats
...
def main():
    parser = argparse.ArgumentParser(description="Baseline Training Runner (SafeLoRA / SaLoRA)")
    parser.add_argument("--task", type=str, required=True, choices=["gsm8k", "alpaca"],
                        help="Task to train on: 'gsm8k' or 'alpaca'")
    parser.add_argument("--method", type=str, required=True, choices=["safelora", "salora"],
    ...
    assert safety_directions_path.exists(), "Safety directions must be extracted (Phase 3) before running baselines!"
